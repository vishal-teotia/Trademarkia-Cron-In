...
- 🎯 Entry points to execute your program using `python -m <trademarkia_cron_in>` or `$ trademarkia_cron_in` with basic CLI argument parsing.
...
---
# trademarkia_cron_in Flask Application

Awesome trademarkia_cron_in created by vishal-teotia
...
```bash
git clone https://github.com/vishal-teotia/Trademarkia-Cron-In trademarkia_cron_in
cd trademarkia_cron_in
make install
```
...
```bash
pip install trademarkia_cron_in
```
...
```bash
trademarkia_cron_in create-db   # run once
trademarkia_cron_in populate-db  # run once (optional)
trademarkia_cron_in add-user -u admin -p 1234  # ads a user
trademarkia_cron_in run
```
